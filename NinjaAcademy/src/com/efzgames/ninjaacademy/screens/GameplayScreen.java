...
public class GameplayScreen extends GameScreen {
	...
	public GameplayScreen(Game game) {
		...
		roomComponent = new StaticTextureComponent(glGame, this, Assets.room,
				Assets.roomRegion, new Vector2(0, 0));
		...
		middleTargetArea = new Rectangle2(
				GameConstants.middleTargetAreaTopLeft,
	...
	private void handleDrag(TouchEvent gesture) {
		...
		if (dragPosition == null) {
			swordSlashOrigin = new Vector2(gesture.x * this.inputScaleX,
					GameConstants.viewPortHeight - gesture.y * this.inputScaleY);
			swordSlashCheckTimer = 0;
			activeSwordSlash = getSwordSlash();
			activeSwordSlash.setStretch(0);
			activeSwordSlash.reset();
		} else {
			dragDistance = FloatMath
					.sqrt((gesture.x * this.inputScaleX - dragPosition.x)
							* (gesture.x * this.inputScaleX - dragPosition.x)
							+ ((GameConstants.viewPortHeight - gesture.y
									* this.inputScaleY) - dragPosition.y)
							* ((GameConstants.viewPortHeight - gesture.y
									* this.inputScaleY) - dragPosition.y));
		...
		dragPosition = new Vector2(gesture.x * this.inputScaleX,
				GameConstants.viewPortHeight - gesture.y * this.inputScaleY);

		activeSwordSlash.positionSlash(new Vector2(swordSlashOrigin), new Vector2(dragPosition));

		if (!sliceComponents(swordSlashOrigin, dragPosition)
				&& dragDistance > audibleDragDistance) {
	...
	private void createTargetComponents() {
		...
		for (int i = 0; i < maxFallingTargets; i++) {
			LaunchedComponent fallingTarget = new LaunchedComponent(glGame,
					this, new Animation(Assets.fallingtarget,
							new Point(57, 58), 8, new Vector2(29, 29), false,
							0.750f));
		...
		for (int i = 0; i < maxFallingGoldTargets; i++) {
			LaunchedComponent fallingGoldTarget = new LaunchedComponent(glGame,
					this, new Animation(Assets.fallinggoldtarget, new Point(57,
							58), 8, new Vector2(29, 29), false, 0.750f));
		...
		for (int i = 0; i < maxConveyerTargets; i++) {
			upperTargetComponents
					.push(getNewConveyerTarget(TargetPosition.Upper));
			middleTargetComponents
					.push(getNewConveyerTarget(TargetPosition.Middle));
			lowerTargetComponents
					.push(getNewConveyerTarget(TargetPosition.Lower));
		...
		for (int i = 0; i < maxGoldTargets; i++) {
			Target goldTarget = new Target(glGame, this, new Animation(
					Assets.goldtarget, new Point(57, 58), 12, new Vector2(29,
							29), true, 0.500f));
	...
	private Target getNewConveyerTarget(TargetPosition targetPosition) {
		Target newTarget = new Target(glGame, this, Assets.target,
				Assets.targetRegion);
	...
	public void update(float deltaTime) {
		...
		int hitCount = Math
				.min((int) Math.floor(totalGameTime * 5
						/ GameConstants.maxGameTime), 5);
...
//			}
	...
	public void present(float deltaTime) {
		...
		batcher.drawSprite(400, 240, 800, 480, Assets.gamePlayBackgroundRegion);
		batcher.endBatch();
	...
	void throwingStarHit(GameComponent sender) {
		...
		Vector2 throwingStarPosition = new Vector2(throwingStar.getPosition());
		...
		// The throwing star should simply remain on screen, "lodged" into
		// something
		throwingStar.isEnabled = false;
	}
	...
	private void createHUDComponents() {
		...
		hitPointsComponent.setTotalHitPoints(GameConfiguration.playerLives);
		hitPointsComponent.setCurrentHitPoints(GameConfiguration.playerLives);
		...
		scoreComponent.setHighscore(oldHighScore);
	...
	// / <summary>
	// / Creates the bamboo, bomb and dynamite components and adds them to the
	// game's component list. All
	...
	private void createLaunchedComponents() {
		...
		for (int i = 0; i < maxBamboos; i++) {
			LaunchedComponent bamboo = new LaunchedComponent(glGame, this,
					Assets.bamboo, Assets.bambooRegion);
			bamboo.setBoundingBoxInflation(GameConstants.boundingBoxInflationbamboo);
		...
		for (int i = 0; i < maxDynamites; i++) {
			LaunchedComponent dynamite = new LaunchedComponent(glGame, this,
					new Animation(Assets.dynamite, new Point(27, 117), 2,
							new Vector2(9, 69), true, 0.250f));
			dynamite.setBoundingBoxInflation(GameConstants.boundingBoxInflationDynamite);
	...
	void bambooDroppedOutOfScreen(LaunchedComponent bamboo) {
		...
		this.scoreComponent.setScore(Math.max(this.scoreComponent.getScore()
				- GameConfiguration.penaltyPerBambooDrop, 0));
	...
	private void createExplosionComponents() {
		...
		for (int i = 0; i < maxDynamites; i++) {
			DisappearingAnimationComponent explosion = new DisappearingAnimationComponent(
					glGame, this, new Animation(Assets.explosion, new Point(40,
							40), 6, new Vector2(20, 21), true, 0.400f));
	...
	private void createBambooSliceComponents() {
		bambooTopSlices = createBambooSliceComponets(Assets.topSlice,
				Assets.topSliceRegion);
		bambooBottomSlices = createBambooSliceComponets(Assets.bottomSlice,
				Assets.bottomSliceRegion);
		bambooLeftSlices = createBambooSliceComponets(Assets.leftSlice,
				Assets.leftSliceRegion);
		bambooRightSlices = createBambooSliceComponets(Assets.rightSlice,
				Assets.rightSliceRegione);
	}

	private LaunchedComponent[] createBambooSliceComponets(Texture texture,
			TextureRegion region) {
		LaunchedComponent[] componentArray = new LaunchedComponent[maxBambooSlices];

		for (int i = 0; i < maxBambooSlices; i++) {
			LaunchedComponent slicedCompoent = new LaunchedComponent(glGame,
					this, texture, region);
	...
	public void setHitPoints(int value) {
		this.hitPointsComponent.setCurrentHitPoints(value);
	}
	...
	private void manageGamePhase(float gameTime) {
		...
		if (currentPhase.duration >= 0
				&& configurationPhaseTimer >= currentPhase.duration) {
			switchConfigurationPhase();
		}
		...
		upperTargetTimer = managePhaseTargets(gameTime, upperTargetTimer,
				currentPhase.targetAppearanceIntervals[0],
				currentPhase.targetAppearanceProbabilities[0],
				upperTargetComponents, GameConstants.upperTargetOrigin,
				GameConstants.upperTargetDestination);
		middleTargetTimer = managePhaseTargets(gameTime, middleTargetTimer,
				currentPhase.targetAppearanceIntervals[1],
				currentPhase.targetAppearanceProbabilities[1],
				middleTargetComponents, GameConstants.middleTargetOrigin,
				GameConstants.middleTargetDestination);
		lowerTargetTimer = managePhaseTargets(gameTime, lowerTargetTimer,
				currentPhase.targetAppearanceIntervals[2],
				currentPhase.targetAppearanceProbabilities[2],
				lowerTargetComponents, GameConstants.lowerTargetOrigin,
				GameConstants.lowerTargetDestination);

		managePhaseBamboos(gameTime);
		managePhaseDynamites(gameTime);
	}

	private float managePhaseTargets(float gameTime, float timer,
			float interval, double probability, Stack<Target> targetStack,
		...
		if (timer >= interval) {
			...
			// Place a new target on the conveyer belt according to probability
			// and if we haven't exceeded the
			// maximal on-screen amount
			if (targetStack.size() > 0 && random.nextDouble() <= probability) {
				Target addedTarget;

				// Place a golden target instead if appropriate and we did not
				// exceed the total amount
				if (goldTargetComponents.size() > 0
						&& random.nextDouble() <= currentPhase.goldTargetProbablity) {
					addedTarget = goldTargetComponents.pop();
				} else {
					addedTarget = targetStack.pop();
				}

				addedTarget.moveWithVelocity(GameConstants.targetSpeed, origin,
						destination);
	...
	private void managePhaseBamboos(float gameTime) {
		...
		if (bambooTimer >= currentPhase.bambooAppearanceInterval) {
			...
			if (bambooComponents.size() > 0
					&& random.nextDouble() <= currentPhase.bambooAppearanceProbablity) {
				...
				launchedBamboo.Launch(getLaunchPosition(), launchSpeed,
						GameConstants.launchAcceleration,
	...
	private void managePhaseDynamites(float gameTime) {
		...
		if (dynamiteTimer >= currentPhase.dynamiteAppearanceInterval) {
			...
			if (dynamiteComponents.size() > 0
					&& random.nextDouble() <= currentPhase.dynamiteAppearanceProbablity) {
				int dynamiteAmount = getDynamiteAmount();
				dynamiteAmount = Math.min(dynamiteAmount,
						dynamiteComponents.size());

				Assets.playSound(Assets.fuseSound);

				for (int i = 0; i < dynamiteAmount; i++) {
					LaunchedComponent launchedDynamite = dynamiteComponents
							.pop();
					inAirDynamiteComponents.add(launchedDynamite);

					Vector2 launchSpeed = getLaunchSpeed();

					launchedDynamite.Launch(getLaunchPosition(), launchSpeed,
							GameConstants.launchAcceleration,
							getLaunchRotation(launchSpeed));
	...
	private Vector2 getLaunchSpeed() {
		return new Vector2(-150 + (float) (random.nextDouble() * 300),
				-(-500 - (float) (random.nextDouble() * 150)));
	}

	private Vector2 getLaunchPosition() {
		return new Vector2(300 + (float) (random.nextDouble() * 200),
				-(500 - GameConstants.viewPortHeight));
	}
	...
	private void createSwordSlashes() {
		...
		for (int i = 0; i < maxSwordSlashes; i++) {
			SwordSlash swordSlash = new SwordSlash(glGame, this, Assets.slice,
					Assets.sliceRegion);
	...
	private boolean sliceDynamite(Line sliceLine) {
		...
		for (int dynamiteIndex = 0; dynamiteIndex < inAirDynamiteComponents
				.size(); dynamiteIndex++) {
			LaunchedComponent dynamite = inAirDynamiteComponents
					.get(dynamiteIndex);
			...
			for (int edgeIndex = 0; edgeIndex < dynamiteEdges.length; edgeIndex++) {
				...
				if (dynamiteEdges[edgeIndex].getIntersection(sliceLine) != null) {
					...
					// Swap the current dynamite, which we would like to remove,
					// with the last one and remove the
					// end of the list
					int lastIndex = inAirDynamiteComponents.size() - 1;
					inAirDynamiteComponents.set(dynamiteIndex,
							inAirDynamiteComponents.get(lastIndex));
					...
					showExplosion(dynamite.getPosition());

					this.scoreComponent
							.setScore(Math.max(
									this.scoreComponent.getScore()
											- GameConfiguration.penaltyPerExpodeDynamite,
									0));
	...
	private boolean sliceBamboo(Line sliceLine) {
		...
		for (int bambooIndex = 0; bambooIndex < inAirBambooComponents.size(); bambooIndex++) {
			...
			if (edgesSliced >= 2) {
				...
				// Swap the current bamboo, which we would like to remove, with
				// the last one and remove the end
				// of the list
				int lastIndex = inAirBambooComponents.size() - 1;
				inAirBambooComponents.set(bambooIndex,
						inAirBambooComponents.get(lastIndex));
				...
				scoreComponent.setScore(scoreComponent.getScore()
						+ GameConfiguration.pointsPerBamboo);
			}
		}
	...
	private void splitBambooVertically(LaunchedComponent bamboo) {
		...
		toLeftHalfCenter = toLeftHalfCenter.rotate(bamboo.rotation
				* Vector2.TO_DEGREES);
		Vector2 toRightHalfCenter = new Vector2(-toLeftHalfCenter.x,
				-toLeftHalfCenter.y);

		toLeftHalfCenter = toLeftHalfCenter.add(bamboo.getPosition());
		toRightHalfCenter = toRightHalfCenter.add(bamboo.getPosition());
		...
		leftSlice.Launch(toLeftHalfCenter,
				Vector2.add(bamboo.velocity, getSliceVelocityVariation()),
				bamboo.acceleration, bamboo.rotation,
				bamboo.angularVelocity * 0.25f);
		...
		rightSlice.Launch(toRightHalfCenter,
				Vector2.add(bamboo.velocity, getSliceVelocityVariation()),
				bamboo.acceleration, bamboo.rotation,
				bamboo.angularVelocity * 0.25f);
	...
	private void splitBambooHorizontally(LaunchedComponent bamboo) {
		...
		toTopHalfCenter = toTopHalfCenter.rotate(bamboo.rotation
				* Vector2.TO_DEGREES);
		Vector2 toBottomHalfCenter = new Vector2(-toTopHalfCenter.x,
				-toTopHalfCenter.y);

		toTopHalfCenter = toTopHalfCenter.add(bamboo.getPosition());
		toBottomHalfCenter = toBottomHalfCenter.add(bamboo.getPosition());
		...
		topSlice.Launch(toTopHalfCenter,
				Vector2.add(bamboo.velocity, getSliceVelocityVariation()),
				bamboo.acceleration, bamboo.rotation,
				bamboo.angularVelocity * 0.25f);
		...
		bottomSlice.Launch(toBottomHalfCenter,
				Vector2.add(bamboo.velocity, getSliceVelocityVariation()),
				bamboo.acceleration, bamboo.rotation,
				bamboo.angularVelocity * 0.25f);
	...
	private Vector2 getSliceVelocityVariation() {
		return new Vector2(-30 + (float) random.nextDouble() * 60, -10
	...
	private boolean checkForTargetHits(Vector2 hitPosition, List<Target> targets) {
		for (int targetIndex = 0; targetIndex < targets.size(); targetIndex++) {
			...
			if (target.checkHit(hitPosition)) {
				...
				// Remove the target from the appropriate active list and return
				// it to the available target stack
				switch (target.designation) {
				case Upper:
					upperTargetComponents.push(target);
					upperTargetsInMotion.remove(target);
				...
				dropTargetFromPosition(target.getPosition(), target.isGolden);

				scoreComponent
						.setScore(scoreComponent.getScore()
								+ (target.isGolden ? GameConfiguration.pointsPerGoldTarget
										: GameConfiguration.pointsPerTarget));
	...
	private void dropTargetFromPosition(Vector2 initialPosition,
			boolean isGolden) {
		if (!isGolden) {
			LaunchedComponent fallingTarget = fallingTargetComponents[fallingTargetIndex++];

			fallingTarget.resetAnimation();
			fallingTarget.Launch(initialPosition, Vector2.zero,
					GameConstants.fallingTargetLaunchAcceleration, 0);
			...
			fallingGoldTarget.Launch(initialPosition, Vector2.zero,
					GameConstants.fallingTargetLaunchAcceleration, 0);
	...
	private void markGameOver() {
		...
		if (this.scoreComponent.getScore() > oldHighScore)
			((NinjaAcademy) glGame)
					.setHighScore(this.scoreComponent.getScore());

		GameOverComponent comp = new GameOverComponent(glGame,
				this.scoreComponent.getScore() > oldHighScore,
				this.scoreComponent.getScore());
		comp.isEnabled = true;
		comp.isVisible = true;
